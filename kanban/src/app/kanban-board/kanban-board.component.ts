...
export class KanbanBoardComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.loadTasks();
    this.connectedLists = this.columns.map(
      (column) => `cdk-drop-list-${column.title}`
    );
  }
  ...
  drop(event: CdkDragDrop<Task[]>): void {
    if (event.previousContainer === event.container) {
      ...
      const newStatusColumn = this.columns.find(
        (column) =>
          column.title === event.container.id.replace('cdk-drop-list-', '')
      );
      if (newStatusColumn) {
  ...
  getTaskPosition(task: Task): number {
    ...
    const taskIndex = this.getTasksByStatus(task.status).findIndex(
      (t) => t.id === task.id
    );
    return taskIndex;
  }
  ...
  confirmDeleteTask(taskId: number) {
    ...
    this.confirmationAction = () => {
      this.taskService
        .deleteTask(taskId, this.authService.getUserId())
        .subscribe(
          () => {
